...
MLsUserInfo
MlsUserSession::setup_mls_userinfo(const std::string& user,
                                   ...
                                   CipherSuite suite)
{
  ...
  info.keypackage = KeyPackage{
    suite, info.init_key.public_key, leaf_node, {}, info.signing_key
  };
  return info;
}
...
MlsUserSession::create(const MLsUserInfo& info)
{
  // create underlying mls state for the creator
  auto mls_state = State{
    from_ascii(info.group),    info.suite, info.leaf_key, info.signing_key,
    info.keypackage.leaf_node, {}
  };
...
// setup mls state for the creator
std::unique_ptr<MlsUserSession>
MlsUserSession::create_for_welcome(const MLsUserInfo& info,
                                   bytes&& welcome_data)
{
  ...
  try {
    auto mls_state = State{ info.init_key,
                            info.leaf_key,
                            info.signing_key,
                            info.keypackage,
                            welcome,
...
const KeyPackage&
MlsUserSession::get_key_package() const
{
  return user_info.keypackage;
}

bytes
MlsUserSession::fresh_secret() const
{
  return random_bytes(user_info.suite.secret_size());
}

mls::State&
MlsUserSession::get_state()
{
  return mls_state;
}

std::tuple<bytes, bytes>
MlsUserSession::process_key_package(std::vector<uint8_t>&& data)
{
  auto kp = tls::get<KeyPackage>(data);
  auto add_proposal = mls_state.add_proposal(kp);
  auto [commit, welcome, _creator_state] = mls_state.commit(
    fresh_secret(), CommitOpts{ { add_proposal }, true, false, {} }, {});
  auto commit_data = tls::marshal(commit);
  auto welcome_data = tls::marshal(welcome);
  // TODO: should we wait for commit to come back to us before overwriting the
  // state ?
